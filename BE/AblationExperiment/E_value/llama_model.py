...
class KVCacheManager(nn.Module):
    ...
    def tokencache_move(self, cachelen, accept_indices_list):
        ...
        """
        # print(f"accept_indices_list: {accept_indices_list}")
        # print(f"cachelen: {cachelen}")
        ...
        for req_id in range(num_requests):
            ...
            # print(f"src_pos: {src_pos}, accepted: {accepted}")
        ...
        # 批量拷贝数据
        # print(f"Moving {len(req_indices)} tokens from {src_positions} to {tgt_positions}")
        with torch.no_grad():
    ...
    def batchcache_move(self, indices):
        ...
        # print(f"llama batchcache_move sources: {sources} targets: {targets}")
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def draft_move(self,cachelen, accept_indices_list):#草稿模型的接受token的移动
        # print(f"cachelen: {cachelen} accept_indices_list: {accept_indices_list}")
    ...
    def forward(
        ...
        if exec_type == "draft_prefill":
            ...
            # print(f"LlamaModel input_ids: {input_ids.shape}, hidden_states: {hidden_states.shape}, cache_lens: {cache_lens.shape}, tree_mask: {tree_mask.shape}")
        ...
        # print(f"LlamaModel input_ids: {input_ids.shape}, hidden_states: {hidden_states.shape}")

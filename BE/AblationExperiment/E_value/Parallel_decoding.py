...
class Parallel_decoding(EaModel):
    ...
    def param_init(self,Maxrequestnum,max_gen_len):#初始化Kvcahe和相关参数
        ...
        # eagle add
        self.ea_layer.reset_kv(max_request_num=self.Maxrequestnum, head_num=self.head_num, head_size=self.head_size)
...
#更新相关的参数
    ...
    def draft_prune(self, sample_token):
        ...
        cart_candidateslist, tree_candidateslist , tree_attn_masklist ,retrieve_indiceslist,node_num,weight_tensor = generate_candidates(
        ...
        self.weight_list+=weight_tensor.tolist()  
        # torch.cuda.synchronize()
    ...
    def tree_spec_generate(self, requests):
        ...
        with torch.no_grad(): 
            ...
            for out_index in range(0, self.Max_gen_len):
                ...
                acc_num=self.acc_num[:self.batch_size]+1
                self.acc_num_list+=acc_num.tolist()

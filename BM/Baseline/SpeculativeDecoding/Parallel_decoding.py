...
class Parallel_decoding(MedusaModel):
    ...
    def __init__(self,model_path,Node_num=300,medusa_choices=None,threshold=3.5,temperature=0,posterior_alpha=0.09,Device_index=None):
        # 调用父类的构造方法，传入配置参数
        super().__init__()
...
#更新相关的参数
    ...
    def draft_produce(self,hidden_states):  # produce draft sequence based on parallel decoding          
        #hidden_states batch X 1 X hidden_dim
        medusa_logits = []
        ...
        MT=MultiTokenGenerator(self.topk,medusa_choices=self.medusa_choices)
        cart_candidateslist, tree_candidateslist , tree_attn_masklist ,retrieve_indiceslist,node_num=MT.quick_node(medusa_logits,logits.squeeze(),self.medusa_choices)
        ...
        for i in range(self.batch_size):
            ...
            batch_tree_candidates.append(tree_candidates)
            batch_tree_mask.append(medusa_attn_mask)

        batch_tree_candidates = torch.stack(batch_tree_candidates).long().to(self.Model_device)
        batch_tree_mask = torch.stack(batch_tree_mask).to(self.Model_device)#[batch_size,seq_len,seq_len]
    ...
    def tree_spec_generate(self, requests):
        ...
        with torch.no_grad(): 
            ...
            for out_index in range(0, self.Max_gen_len):
                ...
                batch_candidates_list,batch_tree_candidates,batch_tree_mask ,batch_retrieve_indices_list=self.draft_produce(hidden_states)
                ...
                # self.generate_time += time.time() - start_gerate_time

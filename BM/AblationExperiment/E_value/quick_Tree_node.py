...
class MultiTokenGenerator:
    ...
    def quick_node_prune(self,
        ...
        weights = torch.arange(1, 6, dtype=torch.float32,device=cart_candidates_probs.device).view(1, 1, -1)
        weight_cart_candidates_probs=cart_candidates_probs* weights
        Node_probs =  cart_candidates_probs[:, self.node_index[:, 0], self.node_index[:, 1]]
        weight_Node_probs = weight_cart_candidates_probs[:, self.node_index[:, 0], self.node_index[:, 1]]
        weight_tensor=torch.sum(weight_Node_probs, dim=1)
        
        proportions = weight_tensor / weight_tensor.sum()
        allocated = proportions * self.NODE
